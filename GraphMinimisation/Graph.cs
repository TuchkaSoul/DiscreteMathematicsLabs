...
namespace GraphMinimisation
{
    public class Graph : IEnumerable<Rib>
    {
        ...
        public Color[] GetColorableBad()
        {
            Color[] color = new Color[8] { Color.Red, Color.Blue, Color.Green, Color.Yellow, Color.Orange, Color.Purple, Color.Pink, Color.Black };
            Color[] result= new Color[this.GetCountPeek()];
            int ih =this.GetCountPeek();
           
            for (int i = 0; i < 8&& result.Contains(Color.Empty); i++)
            {
                Graph ngc=new Graph(this.GetCountPeek());

                for (int j=0; j< GetCountPeek(); j++)
                {
                    if (result[j].IsEmpty)
                    {
                        if (ngc.grafs[j].Count==0)
                        {
                            result[j] = color[i];
                            color[i].Name.ToString();
                            ngc.Add(grafs[j]);
                        }
                        else
                        {
                            bool isCheck = true;
                            for (int k = 0; k < GetCountPeek() && isCheck; k++)
                            {
                                if (ngc.grafs[k].Count != 0)
                                    isCheck = !this.IsConnect(k, j);
                            }
                            if (isCheck)
                            {
                                result[j] = color[i];
        ...
        public int GetCountPeek() { return grafs.Count-1; }

        public bool IsConnect(int A,int B)
        {
            foreach (var item in ribs) 
            {
                if (item.A == A && item.B == B) return true;
                else if( item.A == B && item.B == A)return true;
                continue;
            }
            return false;
        }
